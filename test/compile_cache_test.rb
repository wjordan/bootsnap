...
class CompileCacheTest < Minitest::Test
  ...
  def test_no_write_permission
    ...
    @handler.expects(:input_to_storage).never
    @handler.expects(:input_to_output).times(2).returns('whatever')
    ENV['OPT_AOT_LOG'] = '1'
  ...
  def test_file_is_only_read_once
    ...
    @handler.expects(:input_to_storage).times(1).returns(storage)
    @handler.expects(:storage_to_output).times(2).returns(output)
    load(path)
    load(path)
  end
  ...
  def test_recache
    ...
    @handler.expects(:input_to_storage).times(2).returns(storage)
    @handler.expects(:storage_to_output).times(2).returns(output)
